...
function StdUi:ApplyScrollTableMethods(object)
	local this = self
	...
	local methods = {
		...
		SetColumns = function(self, cols)
			...
			for i = 1, #cols do
				...
				if not columnHeadFrame.cols[i] then
					columnFrame = this:HighlightButton(columnHeadFrame);
					columnFrame:SetPushedTextOffset(0, 0);

					columnFrame.arrow = this:Texture(columnFrame, 8, 8, [[Interface\Buttons\UI-SortArrow]]);
			...
			for i = 1, num do
				...
				for j = 1, #self.cols do
					...
					if not cell then
						...
						cell.text = this:FontString(cell, '');
		...
		SetDefaultHighlightBlank = function(self, red, green, blue, alpha)
			if not self.defaultHighlightBlank then
				self.defaultHighlightBlank = this.config.highlight.blank;
			end
		...
		SetDefaultHighlight = function(self, red, green, blue, alpha)
			if not self.defaultHighlight then
				self.defaultHighlight = this.config.highlight.color;
			end
			...
			if shouldShow then
				...
				elseif (format == 'icon') then
					if cellFrame.texture then
						...
						cellFrame.texture = this:Texture(cellFrame, iconSize, iconSize, value);
		...
		Refresh = function(self)
			...
			this.FauxScrollFrameMethods.Update(scrollFrame, #self.filtered, self.displayRows, self.rowHeight);

			local o = this.FauxScrollFrameMethods.GetOffset(scrollFrame);
			self.offset = o;
	...
	object.cellEvents = cellEvents
	object.headerEvents = headerEvents
...
function StdUi:ScrollTable(parent, cols, numRows, rowHeight, highlight)
	...
	scrollTable = self:ApplyScrollTableMethods(scrollTable)
